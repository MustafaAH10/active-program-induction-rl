...
export class OpenAICompatibleAgentProvider implements MultimodalAgentProvider{
  ...
  async propose(systemPrompt:string,state:Record<string,unknown>,imagePaths:string[],responseSchema:Record<string,unknown>){
    ...
    try{const response=await fetch(`${this.config.baseUrl.replace(/\/$/,'')}/chat/completions`,{method:'POST',headers:{authorization:`Bearer ${this.config.apiKey}`,'content-type':'application/json'},signal:controller.signal,body:JSON.stringify({model:this.config.model,messages:[{role:'system',content:systemPrompt},{role:'user',content:JSON.stringify({state,imagePaths:imagePaths.map(path=>path.split('/').at(-1))})}],response_format:{type:'json_schema',json_schema:{name:'agent_proposal',strict:true,schema:responseSchema}}})});if(!response.ok)throw new Error(`Provider error ${response.status}`);const raw=await response.text();if(raw.length>2_000_000)throw new Error('Provider response exceeds 2 MB');const data=JSON.parse(raw) as any;return AgentProposalSchema.parse(JSON.parse(data.choices[0].message.content))}finally{clearTimeout(timer)}}
}
